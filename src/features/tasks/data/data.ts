...
export const fakeTasks = {
   records: [] as TaskType[], // Holds the list of product objects
   ...
   async getAll({ priorities = [], statuses = [], labels = [], q }: { priorities?: string[], statuses?: string[], labels?: string[], q?: string }) {
      let tasks = [...this.records]
      ...
      if (labels.length > 0) {
         tasks = tasks.filter((task) =>
            labels.includes(task.label),
         )
      }

      // Search functionality across multiple fields
      if (q) {
         tasks = matchSorter(tasks, q, {
            keys: ['id', 'title'],
         })
      }
   ...
   async getTasks({ page = 1, limit = 10, priorities, statuses, labels, q }: { page?: number, limit?: number, priorities?: string[], statuses?: string[], labels?: string[], q?: string }) {
      // await delay(1000)
   ...
   async getTaskById(id: string) {
      ...
      const task = this.records.find((product) => product.id && product.id === id)
