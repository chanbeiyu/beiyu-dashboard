         ...
         <div className="space-y-2">
            {multiDayEvents.length > 0
               && multiDayEvents.map((event) => {
                  ...
                  return <AgendaEventCard event={event} eventCurrentDay={eventCurrentDay} eventTotalDays={eventTotalDays} key={event.id} />
               })}

            {sortedEvents.length > 0 && sortedEvents.map((event) => <AgendaEventCard event={event} key={event.id} />)}
         </div>
      </div>
   )
}
