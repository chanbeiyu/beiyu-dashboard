      ...
      <div className="hidden overflow-hidden sm:flex">
         ...
         <div className="grid flex-1 grid-cols-7 divide-x border-b border-l">
            {weekDays.map((day, dayIndex) => (
               <div className="flex h-full flex-col gap-1 py-1" key={day.toISOString()}>
                  {eventRows.map((row, rowIndex) => {
                     const event = row.find((e) => e.startIndex <= dayIndex && e.endIndex >= dayIndex)

                     if (!event) {
                        return <div className="h-6.5" key={`${rowIndex}-${dayIndex}`} />
                     }
                     ...
                     return <MonthEventBadge cellDate={startOfDay(day)} event={event} key={`${event.id}-${dayIndex}`} position={position} />
                  })}
               </div>
            ))}
         </div>
